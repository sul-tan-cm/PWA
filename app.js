...
function renderInquiryPage() {
  ...
  let detailHtml = '';
  if (matched.length === 1) {
    detailHtml = renderEmployeeDetail(matched[0]);
    ...
    detailHtml = `<div class="empty-state">لا توجد نتائج مطابقة لـ "${query}"</div>`;
  }
  ...
  return `
    <div class="card">
      ...
      <input type="text" id="searchInput" placeholder="مثال: عبدالله أو 400021" value="${query}" oninput="searchEmployee(this.value)">
    </div>
    ${detailHtml}
  `;
}
...
function selectEmployee(empId) {
  ...
  state.employeeSearchQuery = emp.name;
  render();
}
...
function renderEmployeesPage() {
  ...
  return `
    <div class="card">
      ...
      <div class="tbl-wrap">
        <table>
          <thead><tr><th>الرقم العسكري</th><th>الاسم</th><th>الرتبة</th><th>التخصص</th></tr></thead>
          <tbody>${rows}</tbody>
        </table>
      </div>
    </div>
...
function render(skipFocusFix) {
  ...
  if (state.currentTab === 'inquiry' && !skipFocusFix) {
    // لا شيء إضافي حالياً
  }
